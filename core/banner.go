...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - -- Updated at: 2024.04.29 |  https://t.me/mr_ro_obot  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by EvilHoster Telegram (") + handleClr.Sprintf("@mr_ro_obot") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
...
func Banner() {
	...
	//printOneliner2()
	//fmt.Println()
	putAsciiArt("_   @@@@@@@@@@@@@@ Telegram: mr_ro_obot @@@@@@@@@@@@@@@@@@@   _")
	printOneliner1()
	fmt.Println()
	putAsciiArt("__                                     __\n")
	fmt.Println()
